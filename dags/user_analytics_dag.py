...
from api_client import process_and_save_parquet
from db_loader import load_parquet_to_staging

DATA_DIR = os.path.join(os.path.dirname(__file__), "..", "data")

TRANSFORM_SQL = """
INSERT INTO dim_users (user_uuid, username, title, first_name, last_name, gender, email, phone, cell, nationality)
SELECT u.uuid, u.username, u.title, u.first_name, u.last_name, u.gender, u.email, u.phone, u.cell, u.nationality
...
SELECT u.street_number, u.street_name, u.city, u.state, u.country, u.postcode, u.latitude::NUMERIC, u.longitude::NUMERIC, u.timezone_offset, u.timezone_desc
FROM staging_users u
...
FROM generate_series('2006-01-01'::date, '2026-12-31'::date, interval '1 day') AS d
ON CONFLICT (full_date) DO NOTHING;

INSERT INTO fact_user_registrations (user_uuid, location_key, dob_date_key, reg_date_key, age_at_registration, current_age, registered_years)
SELECT u.uuid, l.location_key, d1.date_key, d2.date_key,
       ...
       (EXTRACT(YEAR FROM AGE(d2.full_date, d1.full_date)))::INT
FROM staging_users u
JOIN dim_locations l ON l.street_number = u.street_number AND l.street_name = u.street_name AND l.postcode = u.postcode
